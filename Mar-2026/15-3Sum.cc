...
public:
  std::vector<std::vector<int>> threeSum(std::vector<int> &nums) {
    ...
    for (size_t first = 0; nums[first] <= 0 && first < nums.size() - 2; first++) {
...
public:
  std::vector<std::vector<int>> threeSumSlow(std::vector<int> &nums) {
    ...
    std::vector<std::vector<int>> new_results;
    for (int first = 0; first < nums.size() - 2;
         handled.insert(handled.end(), nums[first]), first++) {
      if (std::find(handled.begin(), handled.end(), nums[first]) !=
          handled.end()) {
        // If we have handled this number, skip
        continue;
      }

      // It's faster if we insert new results to result directly in function
      // twoSum.  But it will make the purpose of twoSum unclear
      new_results = twoSumSlow(nums, first + 1, -nums[first], handled);
...
private:
  std::vector<std::vector<int>> twoSumSlow(std::vector<int> &nums, int start,
                                           int target,
                                           std::vector<int> &handled) {
    std::vector<std::vector<int>> results = {};
    ...
    // Special case: nums = [-2, 1, 1, 1, 1, 1, 2, 0], start = 1, target = 2
    // because nums[2|3|4|6] = nums[1], we want to skip those duplicate cases
    // latter cases, but if we skip whenever nums[i] exists in handled list,
    // we will skip a important result [-2, 1, 1]
    ...
    for (; current < nums.size(); current++) {
      // Am I handled? If handled, continue
      if (std::find(handled.begin(), handled.end(), nums[current]) !=
          handled.end()) {
        continue;
      }
      if (std::find(demands.begin(), demands.end(), target - nums[current]) !=
          demands.end()) {
        // Special case: demand
        if (!special_case && nums[current] * 2 == target) {
          special_case = true;
        }
        continue;
      }
      ...
      // Am I needed?
      if (std::find(demands.begin(), demands.end(), nums[current]) !=
          demands.end()) {
        // Yeah, someone needs me!
        results.insert(results.end(), {nums[start - 1], target - nums[current],
                                       nums[current]});
      }

      // If my demand is listed, this line will be skiped, so we don't need
      // if-statement If my demand is not listed, add my demand
      demands.insert(demands.end(), target - nums[current]);
    }

    if (special_case) {
      results.insert(results.end(), {nums[start - 1], target / 2, target / 2});
    }
    return results;
  }

public:
  std::vector<std::vector<int>> threeSumAllResults(std::vector<int> &nums) {
    std::vector<std::vector<int>> results = {};
    if (nums.size() < 3) {
      return results;
    }
    ...
    for (size_t first = 0; first < nums.size() - 2; first++) {
      ...
      // twoSum.  But it will make the purpose of twoSum unclear
      std::vector<std::vector<int>> new_results =
          twoSumAllResults(nums, first + 1, -nums[first]);
      results.insert(results.end(), new_results.begin(), new_results.end());
    }
    return results;
  }
...
private:
  std::vector<std::vector<int>> twoSumAllResults(std::vector<int> &nums,
                                                 size_t start, int target) {
    std::vector<std::vector<int>> result = {};
    if (nums.size() - start < 2) {
      return result;
    }
    ...
    for (; current < nums.size(); current++) {
      ...
      if (demand_map.find(nums[current]) != demand_map.end()) {
        ...
        for (size_t _ = 0; _ < demand_map[nums[current]].size(); _++) {
          result.insert(result.end(),
                        {nums[start - 1], nums[demand_map[nums[current]][_]],
                         nums[current]});
          // It's safe to convert size_t to int
          // first (in threeSum()) = start - 1
        }
      }
...
int main() {
  ...
  std::vector<int> nums = {1, 2, 0, 1, 0, 0, 0, 0, 0};
  std::vector<std::vector<int>> results = demo.threeSum(nums);
